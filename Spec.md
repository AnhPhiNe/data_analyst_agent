...
- Validate extension, size, encoding, delimiter, and structural readability. An XLSX upload is checked by its ZIP signature and workbook structure. A CSV has no file signature, so its content must decode as text without NUL bytes and must not begin like a known binary format, such as an XLSX renamed to `.csv`; a browser-supplied MIME type is not trusted as evidence.
...
- Inconsistent-spelling warnings for text values that differ only by letter case or surrounding spaces, with up to three examples; values are reported, never rewritten, and possible PII fields are flagged without examples.
...
Besides the model's drafts, synthesis deterministically reports each statistical result's headline estimates — every group mean for a group comparison, or the single coefficient for a correlation or regression — that the model did not already assert. Likewise, once the model asserts a row value from a complete grouped or single-row query result of at most 20 values, synthesis reports that result's other values, leaving out group labels, `row_number`, empty values, and results computed from possible PII, so a correct answer is not stated in part. These extra Verified Insights use the same verification and deterministic rendering, so headline statistical values are covered regardless of model variance.
...
- Numeric tolerances.
- Allowed fields. Allowed filters are recorded for review but are not graded automatically.
- Supported conclusions, recorded for the manual rubric review rather than for automatic grading.
- Forbidden claims.
...
Each case declares its expected outcome — answered with Verified Insights, answered from the Data Profile, a clarification question, or a safe refusal — and may list other acceptable outcomes. Clarification behavior is graded through the outcome: the runner approves plans but never confirms a clarification on the user's behalf. The evaluation runner (`python -m tabular_analytics_agent.evaluation.runner`) runs the cases and paces model calls to the provider quota; the grading module grades outcome, computed values, values reported in Verified Insights, schema grounding against allowed fields, chart type, and forbidden claims, and reports each Section 17.4 gate.

### 17.3 Evaluation process

Grading version 3 matches an expected grouped query value by its group identity and value, whatever output name the model chose, and never counts a `GROUP BY` label column as the measured value. An ungrouped expected value matches a single-row result, or a multi-row result only through the named metric or an explicitly allowed alias. Statistical metrics use their exact metric identifiers; since version 4, JSON-encoded group labels inside them are compared after decoding, so `group[str:"Có"]` matches its ASCII-escaped spelling. Version 3 replaced alias-list matching after correct values under unlisted aliases (for example `departed_count` or `avg_1`) were graded as missing. Insight coverage requires a persisted assertion operand and evidence from the same successful Tool Action/result, not merely a matching number elsewhere. Profile cases declare concrete expected facts. Failed runs and rejected insight drafts do not count as safe refusals. Refusal credit requires a typed refusal code and a non-empty reason: `unavailable_metric` also requires an unavailable mapping, and `insufficient_sample` comes only from the statistical tool's typed sample-size error. Other failure categories never receive refusal credit, and error messages are never matched as text. Earlier grading reports are not directly comparable with version 2. Grading version 5 keeps every per-run check and adds a table of the Section 17.4 gates, each with its own numerator and denominator: expected values in answered cases, fields read by successful Tool Actions, Verified Insights (for forbidden claims, and for Evidence Trails, where an insight counts as evidenced when it names its Tool Actions), runs with an analysis plan that did not end in failure, runs of cases that list valid charts, and runs of clarification cases. Unnecessary clarifications are counted separately, and a gate with no qualifying run is reported as not measured. Re-grading the stored holdout v5–v9 runs with version 5 changed no run's pass or fail relative to version 4. Grading version 6 lets an expected value name several group columns, or a group by value alone when the group is a derived expression such as a normalized label whose output name the model chooses, and lets a case list alternative calculation sets for equally valid methods, such as a t-test's group means or a Mann–Whitney test's medians; the set the run matched best is graded. This deterministic grader does not replace semantic review of metric meaning, filters, or the rendered UI. Because a grouped or single-row expected value is matched by its group and value rather than by the model's output name, a different column holding the same value also counts; re-checking the stored release runs with a strict name check (the column must carry the expected metric name or an alias, or be the result's only measured column) found 151 of 176 expected values instead of 155, so calculation accuracy reads 85.8% instead of 88.1%, and only 2 of 118 runs of one case were affected. The official grades are not recomputed.
...
- Fixes from a full project review, none of which changes a prompt template or grading rule: Streamlit explains a rejected upload, a failed profiling, pin, publication, or run instead of surfacing a traceback (FR-02 and Section 18); a Candidate Artifact can be shown as another supported chart type over the same verified result, re-rendered from the checkpoint history without a model call, which completes Section 21 criterion 8 (FR-11); a CSV whose content begins like a known binary format is refused by name (FR-02); a statistical parameter error, such as a group order the data does not contain, is repaired by the model while data that cannot support the analysis still stops the run (Section 15); evaluation token totals keep a retried provider error's tokens (Section 16); the Kruskal–Wallis statistic has a claim label, guarded by a test that every statistic the engine reports has one (FR-10); the Data Overview explorer states when no rows match instead of showing zero (FR-11); viewing a stored result no longer rewrites the session file; and the stale-insight model, an unused SQL helper, duplicated identifier parsing, and repeated literal thresholds were removed or given one definition. Grading leniency and the fields recorded but not graded are documented in Section 17.2 and `docs/limitations.md` Section 9.
